...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('search-btn').addEventListener('click', searchManager);
...
function searchManager() {
    let searchId = document.getElementById('search-manager-id').value;
    if (searchId) {
      fetch(`http://localhost:8000/managers/${searchId}`, {
        method: 'GET',
      })
      .then(response => response.json())
      .then(data => {
        ...
        displayData(data);
          ...
          alert('No Manager found with the provided ID');
        }
      })
